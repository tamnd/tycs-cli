...
describes what users get. Pick a format with `-o`, or let tycs choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
tycs <command> -o table   # aligned columns for reading
tycs <command> -o jsonl   # one JSON object per line, for piping
tycs <command> -o json    # a single JSON array
tycs <command> -o csv     # spreadsheet friendly
tycs <command> -o tsv     # tab-separated
tycs <command> -o url     # just the URL column
tycs <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
tycs <command> --fields id,title,url
```
...
```bash
tycs <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
tycs <command>            # a table, because this is a terminal
tycs <command> | wc -l    # JSONL, because this is a pipe
```
