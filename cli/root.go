// Package cli assembles the tycs command tree from the tycs
...
// NewApp assembles the kit application from the tycs domain. The
...
//
// To add a command, declare it in tycs/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := tycs.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(tycs.Domain{}).Register(app)
